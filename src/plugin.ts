        ...
        loader: async (
          ...
          if (
            ...
            if (!hasMatchingAccount) {
              ...
              log.warn(
                "Stored accounts don't match OpenCode's auth. Clearing stale storage.",
              );
              try {
                await clearAccounts();
              } catch {
            ...
            async fetch(input, init) {
              ...
              while (true) {
                ...
                if (!account) {
                  ...
                  const waitMs =
                    accountManager.getMinWaitTimeForFamily(family, model) ||
                    60_000;
                  ...
                  for (
                    ...
                    try {
                      ...
                      if (!quietMode && prepared.thinkingRecoveryMessage) {
                        await showToast(
                          prepared.thinkingRecoveryMessage,
                          "warning",
                        );
                      }
                      ...
                      if (response.status === 429) {
                        ...
                        if (isCapacityExhausted) {
                          ...
                          if (
                            ...
                            if (hasOtherAccountWithAntigravity(account)) {
                              pushDebug(
                                `capacity exhausted on account ${account.index}, but available on others. Switching account.`,
                              );
                              shouldSwitchAccount = true;
                              break;
                            }
                        ...
                        if (delayMs <= SHORT_RETRY_THRESHOLD_MS) {
                          await showToast(
                            `Rate limited. Retrying in ${waitTimeFormatted} (attempt ${attempt})...`,
                            "warning",
                          );
                          await sleep(delayMs, abortSignal);
                          continue;
                        ...
                        if (accountCount > 1) {
                          ...
                          await showToast(
                            `Rate limited on ${quotaName} quota for ${accountLabel}${quotaMsg}. Switching account...`,
                            "warning",
                          );
                          ...
                          await showToast(
                            `Rate limited. ${quotaMsg} (attempt ${attempt})...`,
                            "warning",
                          );
