...
export const createAntigravityPlugin = (providerId: string) => async (
  ...
  const checkQuotaTool = tool({
    description: "Check the current quota status and rate limits for all authenticated Antigravity accounts. Returns detailed usage statistics for Claude and Gemini models, including remaining quota percentages and reset times.",
    args: {},
    async execute(_args, _ctx) {
      if (!activeAccountManager) {
        return "Error: Not authenticated. Please run `opencode auth login` first.";
      }
      
      const accounts = activeAccountManager.getAccounts();
      if (accounts.length === 0) {
        return "Error: No accounts configured. Please run `opencode auth login` to add accounts.";
      }

      log.debug("check_quota tool called", { accountCount: accounts.length });
      
      // Convert ManagedAccount[] to AccountMetadataV3[] for quota check
      // We need to extract refreshToken/projectId from 'parts' property
      const accountsForCheck = accounts.map(acc => ({
        ...acc,
      ...
      // Fetch fresh quotas
      const results = await checkAccountsQuota(accountsForCheck, client, providerId);
      ...
      // Format output
      let output = "### 📊 Quota Report\n\n";

      // Helper for reset time formatting
      const formatResetTime = (timeStr?: string): string => {
        if (!timeStr) return "";
        const ms = Date.parse(timeStr) - Date.now();
      ...
      for (const res of results) {
        const email = res.email || `Account ${res.index + 1}`;
        const statusIcon = res.status === "ok" && !res.disabled ? "✅" : "❌";
        const statusText = res.disabled ? "Disabled" : res.status === "ok" ? "Active" : "Error";
        ...
        // Gemini CLI Quota
        const cliModels = res.geminiCliQuota?.models || [];
        if (cliModels.length > 0) {
          output += "**Gemini CLI Quota**\n";
          for (const model of cliModels) {
            const percent = Math.round(model.remainingFraction * 100);
            const resetTime = formatResetTime(model.resetTime);
            // Highlight low quota
            const warning = percent < 20 ? " ⚠️" : "";
            output += `* \`${model.modelId}\`: **${percent}%**${warning}${resetTime}\n`;
          }
          output += "\n";
        } else if (res.geminiCliQuota?.error) {
             output += `**Gemini CLI Quota**: ${res.geminiCliQuota.error}\n\n`;
        }

        // Antigravity Quota
        const groups = res.quota?.groups;
        if (groups && Object.keys(groups).length > 0) {
           output += "**Antigravity Quota**\n";
           const entries = [
           ...
           for (const entry of entries) {
             ...
             const percent = Math.round((entry.data.remainingFraction ?? 0) * 100);
             const resetTime = formatResetTime(entry.data.resetTime);
             const warning = percent < 20 ? " ⚠️" : "";
             output += `* \`${entry.name}\`: **${percent}%**${warning}${resetTime}\n`;
           }
           output += "\n";
        } else if (res.quota?.error) {
            output += `**Antigravity Quota**: ${res.quota.error}\n\n`;
        }
