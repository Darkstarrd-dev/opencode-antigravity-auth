...
export async function executeImageGeneration(
  ...
  try {
    ...
    if (!response.ok) {
      ...
      log.debug("Image generation API error", {
    ...
    log.debug("Image generation response received", { hasResponse: !!data.response });

    return parseImageResponse(data, workingDirectory, requestPayload);
  } catch (error) {
    const message = error instanceof Error ? error.message : String(error);
    log.debug("Image generation execution error", { error: message });

    // Log failure
    try {
      const imgsDir = ensureImgsDirectory(workingDirectory);
      appendToListMd(imgsDir, "", requestPayload, false, message);
    } catch {
      // Ignore
    }
    ...
    return `Error: Failed to generate image - ${message}. Please try again.`;
  }
}
